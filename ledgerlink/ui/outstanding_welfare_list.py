from ledgerlink.models import OutstandingWelfare
from xf.xf_crud.model_lists import XFModelList
from xf.xf_crud.xf_classes import XFUIAction, ACTION_ROW_INSTANCE

class OutstandingWelfareList(XFModelList):

    def __init__(self, model):
        super(OutstandingWelfareList, self).__init__(model)
        ...
        self.list_title = "Outstanding Welfare"
        self.search_hint = "Search....."
        self.supported_crud_operations.append('search')
        self.list_hint = "Below is a list of outstanding welfare"
    ...
    def get_queryset(self, search_string, model, preset_filter, view_kwargs=None):
        if view_kwargs is not None:
            if "related_fk" in view_kwargs:
                return OutstandingWelfare.objects.filter(Meeting_id=view_kwargs['related_fk'])
        return OutstandingWelfare.objects.all()